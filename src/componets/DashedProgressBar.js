...
const DashedGradientProgressBar = ({
  percent,
  width,
  height,
  stopColors,
  ...
  return (
    <AnimatedSvg height={`${height}`} width={`${width}`}>
      <LinearGradient
        id="grad"
        x1="0%"
        x2="100%"
      ...
      <Defs>
        <G x="0" y="0">
          <ClipPath id="clip">
            <AnimatedRect x={translateX} y={'0'} width="100%" height="100%" />
          </ClipPath>
        </G>
      </Defs>
      <Line
        ...
        stroke="url(#grad)"
        strokeWidth="100%"
        strokeDasharray={`${segmentWidth} ${gapWidth}`}
      />
      <Line
        x1="0"
        y1="0"
...
DashedGradientProgressBar.defaultProps = {
  ...
  duration: 300,
  gapWidth: 5,
  stopColors: [<Stop key={1} offset="100%" stopColor="#FFFFFF" />],
};
